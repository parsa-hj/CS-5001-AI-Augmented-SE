...
log = logging.getLogger("localclaw.email")
log.setLevel(logging.INFO)

fmt = logging.Formatter("%(message)s")

# File handler
fh = RotatingFileHandler("email_gateway.log", maxBytes=1_000_000, backupCount=3)
...
def api_emails():
    ...
    email_only = [e for e in all_items if e.get("channel") == "email"]
    return jsonify(email_only[:limit])
...
def run():
    ...
    log.info("=" * 60)
    log.info("✉️   LocalClaw Email Gateway")
    ...
    print(f"\n  ✅  LocalClaw Email Gateway is online")
